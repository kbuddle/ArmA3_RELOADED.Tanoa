/*
	...
	Name: fn_serviceLandVehicleSetup.sqf
	Version: 1.0
...
RELO_fncl_serviceLandVehicleSetup = {
	_servicePads = (allMissionObjects "ALL") select {["servicePadLandVehicle", str _x] call BIS_fnc_inString};
	...
	{
		{
			...
			_servicePadTrigger setTriggerStatements [ \
				"((vehicle player) in thisList) && {(_x isKindOf 'LandVehicle') && (isTouchingGround _x) && (alive _x) && (!isNull assignedDriver _x)} count thisList > 0", \
				"serviceLandVehicle = player addAction ['Service Land Vehicle', {[_this select 1] spawn RELO_fnc_serviceLandVehicle},[],0,true,false,'','((!isNull (assignedDriver vehicle _this)) && (isTouchingGround vehicle _this))',-1,false,''];", \
				"player removeAction serviceLandVehicle; ['', 1] call RELO_fnc_titleTextGlobal" \
			];
			if (isServer) then
			{
				...
				_servicePadMarker setMarkerColor "colorOrange";
				_servicePadMarker setMarkerAlpha 1;
				diag_log format ["[INIT] -> Service Land Vehicle @ Grid Position %1", mapGridPosition _servicePad];
			};
		} forEach _servicePads;
	};
};

publicVariable "RELO_fncl_serviceLandVehicleSetup";
[] call RELO_fncl_serviceLandVehicleSetup;
remoteExecCall ["RELO_fncl_serviceLandVehicleSetup",-2,true];
