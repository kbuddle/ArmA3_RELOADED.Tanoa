...
RELO_fncl_servicePlaneSetup = {
	_servicePads = (allMissionObjects "ALL") select {["servicePadPlane", str _x] call BIS_fnc_inString};
	...
	{
		{
			...
			_servicePadTrigger setTriggerStatements [ \
				"((vehicle player) in thisList) && {(_x isKindOf 'Plane') && (isTouchingGround _x) && (alive _x) && (!isNull assignedDriver _x)} count thisList > 0", \
				"servicePlane = player addAction ['Service Plane', {[_this select 1] spawn RELO_fnc_servicePlane},[],0,true,false,'','((!isNull (assignedDriver vehicle _this)) && (isTouchingGround vehicle _this))',-1,false,''];", \
				"player removeAction servicePlane; ['', 1] call RELO_fnc_titleTextGlobal;" \
			];
			if (isServer) then
			{
				...
				_servicePadMarker setMarkerColor "colorOrange";
				_servicePadMarker setMarkerAlpha 1;
				diag_log format ["[INIT] -> Service Plane @ Grid Position %1", mapGridPosition _servicePad];
			};
		} forEach _servicePads;
	};
};

publicVariable "RELO_fncl_servicePlaneSetup";
[] call RELO_fncl_servicePlaneSetup;
remoteExecCall ["RELO_fncl_servicePlaneSetup",-2,true];
