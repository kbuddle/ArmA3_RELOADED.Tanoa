/*
	...
	Parameter(s):
		...
		distance = number (from how far quadbike comes from) DEFAULT: 1000
		direction = "random" or 0-360 (direction where quadbike comes from, use quotes with random!) DEFAULT: "random"
		distance = number (how many meters before target location units starts jumping out) DEFAULT: 100
		group size = number (how many units is in quadbike drop group) DEFAULT: 6
...
{
	// #ifdef __DEBUG__
		// format ["   ! quadbike troop respawn at task %1 is water position, readjusting to land position...",_task] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
		// diag_log format ["   ! quadbike troop respawn at task %1 is water position, readjusting to land position...",_task];
	// #endif
	...
	// #ifdef __DEBUG__
		// format ["   ! quadbike troop respawn at task %1 readjusted to land position!",_task] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
		// diag_log format ["   ! quadbike troop respawn at task %1 readjusted to land position!",_task];
	// #endif
};
_deployPos = _deployPosRel;
///////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef __DEBUG__
	format ["   + quadbike troop respawn at task %1 with %2 units",_task,_deployNum] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
	diag_log format ["   + quadbike troop respawn at task %1 with %2 units",_task,_deployNum];
#endif
///////////////////////////////////////////////////////////////////////////////////////////////////
...
while {sleep 0.5; _i < 100} do
{
	...
	if !(isNil "_vehiclePosRandom") then
	{
		...
		if (_anyPlayersAround isEqualTo 0) then
		{
			...
			if !(_nearRoads isEqualTo []) then
			{
				...
				if (isOnRoad _nearRoadPos) then
				{
					...
					// #ifdef __DEBUG__
						// format ["   + quadbike troop respawn at task %1 took %2 loop(s) to get spawn location",_task,_i] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
						// diag_log format ["   + quadbike troop respawn at task %1 took %2 loop(s) to get spawn location",_task,_i];
					// #endif
					_i = 101;
				};
			};
...
{
	diag_log "[DEPLOY QUADBIKE] -> unable to find road start position for deploy quadbike spawn, exited!";
};
///////////////////////////////////////////////////////////////////////////////////////////////////
_vehicle = objNull;
...
while {sleep 0.5; !_vehicleSpawnOK} do
{
	...
	{
		// #ifdef __DEBUG__
			// format ["   + quadbike troop respawn engine started, quadbike %1 deployed",_vehicle] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
			// diag_log format ["   + quadbike troop respawn engine started, quadbike %1 deployed",_vehicle];
		// #endif
	...
	{
		// #ifdef __DEBUG__
			// format ["   + quadbike troop respawn engine dead, deleting quadbike %1",_vehicle] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
			// diag_log format ["   + quadbike troop respawn engine dead, deleting quadbike %1",_vehicle];
		// #endif
...
_grpDeploy setGroupIdGlobal [format ["Quadbike Troop Patrol - %1",_groupID]];
...
_grpDeploy setBehaviour "STEALTH";
...
{	
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////
	_grpDeploy leaveVehicle _vehicle; // make AI get out from quadbike and abandon it
	///////////////////////////////////////////////////////////////////////////////////////////////////
	{
		_unit = _x;
...
[_task,_deployPosOriginal,_deploySize,_grpDeploy] spawn
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	_scriptName = format ["RELO_fnc_deployQuadbikeTroop: Task %1, Group %2",_task,_grpDeploy];
