...
_crate enableRopeAttach false; // so helicopters can't sling load it.
_crate allowDamage false; // so the crate can not be damaged.

_cratePos = getPosATL _crate;
_crateDir = getDir _crate;

// so the arsenal crate doesn't move anywhere (vehicles have no collision);
// _roadCone = createSimpleObject ["RoadCone_F",[(_cratePos select 0),(_cratePos select 1),(_cratePos select 2)]];
// _roadCone setDir _crateDir;
// _crate attachTo [_roadCone, [0,0,3.25]];
// _crate attachTo [_roadCone, [0,0,0]];
...
[
	...
	"MRAWS_HEAT_F", // MAAWS HEAT 75 Rocket
	"MRAWS_HE_F", // MAAWS HE 44 Rocket
...
[_crate,(_headgear + _goggles + _uniforms + _vests + _weaponAccessories + _optics + _muzzles + _binoculars + _bipods + _items),true,true] call BIS_fnc_addVirtualItemCargo;
[_crate,(_magazines + _throwables + _mines),true,true] call BIS_fnc_addVirtualMagazineCargo;
[_crate,(_primaryWeapons + _secondaryWeapons + _launcherWeapons),true,true] call BIS_fnc_addVirtualWeaponCargo;

// [_crate] spawn
// {
	// params ["_crate"];
	// waitUntil {sleep 1; (!isNull _crate)};
	// sleep 10;
	// _cratePos = getPos _crate;
	// _crateDir = getDir _crate;
	// _crateVectorUp = vectorUp _crate;
	// while {sleep 1; true} do
	// {
		// _cratePosNow = getPos _crate;
		// _crateDirNow = getDir _crate;
		// _crateVectorUpNow = vectorUp _crate;
		// if (!(_cratePos isEqualTo _cratePosNow) || !(_crateDir isEqualTo _crateDirNow) || !(_crateVectorUp isEqualTo _crateVectorUpNow)) then
		// {
			// _crate setPos _cratePos;
			// _crate setDir _crateDir;
			// _crate setVectorUp _crateVectorUp;
		// };
	// };
// };
