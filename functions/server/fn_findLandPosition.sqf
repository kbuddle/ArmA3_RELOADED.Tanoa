...
_locationDescription = "land";
_locationMarkerColor = "colorGreen";
...
for "_i" from -_mw to _mw step _locationMkrSteps do
{
	for "_j" from -_mh to _mh step _locationMkrSteps do
	{
		...
		{
			_isFlatEmpty = !(_mapPosCurr isFlatEmpty [_maxRadius, -1, _maxGradient, _maxRadius, 0, false] isEqualTo []);
			if (_isFlatEmpty) then
			{
				...
				///////////////////////////////////////////////////////////////////////////////////////////////
				if ((_isOnShore isEqualTo false) && (_isOnRoad isEqualTo false) && (_isNearRoad isEqualTo false) && (_isBaseCompositionClose isEqualTo false)) then
				{
					if (_mapPosCurr inArea _terrainMarker) then
					{
...
{
	RELO_var_locationLandArr = _terrainPosArr;
};
