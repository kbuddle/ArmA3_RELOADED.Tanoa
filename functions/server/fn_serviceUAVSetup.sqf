...
RELO_fncl_serviceUAVSetup = {
	_servicePads = (allMissionObjects "ALL") select {["servicePadPlane", str _x] call BIS_fnc_inString}; // service point is the same as planes.
	...
	{
		{
			...
			_servicePadTrigger setTriggerStatements [ \
				"((vehicle cameraOn) in thisList) && {((_x isKindOf 'UAV_01_base_F') || (_x isKindOf 'UAV_02_base_F') || (_x isKindOf 'UAV_05_Base_F') || (_x isKindOf 'UAV_06_Base_F')) && (isTouchingGround _x) && (alive _x) && (!isNull assignedDriver _x)} count thisList > 0", \
				"vehicleUAV = (vehicle cameraOn); serviceUAV = vehicleUAV addAction ['Service UAV', {[_this select 1] spawn RELO_fnc_serviceUAV},[],0,true,false,'','((!isNull (assignedDriver vehicle _this)) && (isTouchingGround vehicle _this))',-1,false,''];", \
				"vehicleUAV removeAction serviceUAV; ['', 1] call RELO_fnc_titleTextGlobal;" \
			];
			if (isServer) then
			{
				...
				_servicePadMarker setMarkerColor "colorOrange";
				_servicePadMarker setMarkerAlpha 1;
				diag_log format ["[INIT] -> Service UAV @ Grid Position %1", mapGridPosition _servicePad];
			};
		} forEach _servicePads;
	};
};

publicVariable "RELO_fncl_serviceUAVSetup";
[] call RELO_fncl_serviceUAVSetup;
remoteExecCall ["RELO_fncl_serviceUAVSetup",-2,true];
