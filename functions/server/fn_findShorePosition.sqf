...
_locationDescription = "shore";
_locationMarkerColor = "colorYellow";
...
for "_i" from -_mw to _mw step _locationMkrSteps do
{
	for "_j" from -_mh to _mh step _locationMkrSteps do
	{
		...
		if (_mapPosCurr inArea _terrainMarker) then
		{
			...
			if (((surfaceType [_mapPosCurr select 0,_mapPosCurr select 1] isEqualTo "#GdtBeach") && !(surfaceIsWater _mapPosCurr)) || ((surfaceIsWater _mapPosCurr) && (_mapPosCurrDepth >= 0) && (_mapPosCurrDepth <= 0.5))) then
			{
				// if (_mapPosCurr inArea _terrainMarker) then
				// {
...
{
	RELO_var_locationShoreArr = _terrainPosArr;
};
