/*
	...
	Name: fn_serviceHelicopterSetup.sqf
	Version: 1.0
...
RELO_fncl_serviceHelicopterSetup = {
	_servicePads = (allMissionObjects "ALL") select {["servicePadHelicopter", str _x] call BIS_fnc_inString};
	...
	{
		{
			...
			_servicePadTrigger setTriggerStatements [ \
				"((vehicle player) in thisList) && {(_x isKindOf 'Helicopter') && (isTouchingGround _x) && (alive _x) && (!isNull assignedDriver _x)} count thisList > 0", \
				"serviceHelicopter = player addAction ['Service Helicopter', {[_this select 1] spawn RELO_fnc_serviceHelicopter},[],0,true,false,'','((!isNull (assignedDriver vehicle _this)) && (isTouchingGround vehicle _this))',-1,false,''];", \
				"player removeAction serviceHelicopter; ['', 1] call RELO_fnc_titleTextGlobal" \
			];
			if (isServer) then
			{
				...
				_servicePadMarker setMarkerColor "colorOrange";
				_servicePadMarker setMarkerAlpha 1;
				diag_log format ["[INIT] -> Service Helicopter @ Grid Position %1", mapGridPosition _servicePad];
			};
		} forEach _servicePads;
	};
};

publicVariable "RELO_fncl_serviceHelicopterSetup";
[] call RELO_fncl_serviceHelicopterSetup;
remoteExecCall ["RELO_fncl_serviceHelicopterSetup",-2,true];
