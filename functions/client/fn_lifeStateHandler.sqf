...
player addEventHandler ["Dammaged",{
	...
	{
		_isPilot = ((player getUnitTrait "RELO_trait_helicopterPilot") || (player getUnitTrait "RELO_trait_fighterPilot"));
		if (_isPilot) then
		{
			player setDammage 1; // forces respawn at base for pilots on incapacitated state
		};
	};
}];
