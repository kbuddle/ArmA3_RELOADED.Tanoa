...
if !(RELO_var_servicingVehicle) then
{
	...
	if (RELO_var_servicingVehicle) then
	{
		...
		if ((_vehicleDamage > 0) || (_vehicleDamageHitPointDamage > 0)) then
		{
			...
			uiSleep 1;
			if !(RELO_var_servicingVehicle) exitWith {};
			_text = format ["Repairing Body Work...", _hitPointCount, (count _vehicleHitPointsNameArr)];
			[_text, 1] call RELO_fnc_titleTextGlobal;
			uiSleep 5;
			if !(RELO_var_servicingVehicle) exitWith {};
		...
		///////////////////////////////////////////////////////////////////////////////////////////////
		if !(RELO_var_servicingVehicle) exitWith {};
		// doesnt work if magazine is empty!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		_vehicleTurrets = magazinesAllTurrets _vehicle;
		// [["168Rnd_CMFlare_Chaff_Magazine",[-1],168,1.00046e+007,2],["2000Rnd_65x39_Belt_Tracer_Red",[1],1829,1.00046e+007,2],["2000Rnd_65x39_Belt_Tracer_Red",[2],1697,1.00046e+007,2]]
		...
		{
			{
				...
				if (_turretNeedsRearm) then
				{
					...
					{
						if !(_turretUnit isEqualTo _unit) then
						{
							moveOut _turretUnit;
