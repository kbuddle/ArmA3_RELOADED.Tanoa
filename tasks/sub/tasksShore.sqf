...
///////////////////////////////////////////////////////////////////////////////////////////////////
#include "tasksShoreConfig.sqf"; // configuration for tasks.
///////////////////////////////////////////////////////////////////////////////////////////////////
_taskChosen = [];
...
if ((_taskChosen select 0) in RELO_var_locationTasksActive) exitWith
{
	...
	#endif
	if (isNil "noSuitableTasksShore") then
	{
		noSuitableTasksShore = 1;
	}
	else
	{
		noSuitableTasksShore = noSuitableTasksShore + 1;
		if (noSuitableTasksShore > count _taskArr) exitWith
		{
			RELO_var_locationTaskAmount = RELO_var_locationTaskAmount - 1;
			noSuitableTasksShore = nil;
			#ifdef __DEBUG__
				"  ! exhausted suitable shore tasks, adjusted total task amount!" remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log "  ! exhausted suitable shore tasks, adjusted total task amount!";
			#endif
		};
	};
};
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// _clearSurround = nearestTerrainObjects [_taskLocation,["Tree","Small Tree","Bush","Rock","Rocks","Hide"],(_taskAreaBase + 10)];
...
_shoreLineArr = [];
_positionsArr = [];
_positionCurr = _taskLocation;
_position = [0,0,0];
_distance = (_taskAreaBase * 2);
_orgX = _positionCurr select 0;
_orgY = _positionCurr select 1;
_initialPosition = "";
_gotInitialPosition = false;
_lastPosition = "";
_gotLastPosition = false;
for [{_direction = 0},{_direction < 360},{_direction = _direction + 5}] do
{
	_positionLast = _position;
	_posX = _orgX + (_distance * sin _direction);
	_posY = _orgY + (_distance * cos _direction);
	_position = [_posX,_posY,0];
	_isSurfaceBeach = (((surfaceType _position) isEqualTo "#GdtBeach") && !(surfaceIsWater _position));
	_isSurfaceWater = (surfaceIsWater _position);
	if (_initialPosition isEqualTo "") then
	{
		if (_isSurfaceBeach isEqualTo true) then
		{
			_initialPosition = "beach";
		};
		if (_isSurfaceWater isEqualTo true) then
		{
			_initialPosition = "water";
		};
	};
	if (_gotInitialPosition isEqualTo false) then
	{
		if ((_initialPosition isEqualTo "beach") && (_isSurfaceWater isEqualTo true) && !(_position isEqualTo [0,0,0])) then
		{
			#ifdef __DEBUG__
				format ["   + beach start - first beach position is %1",_positionLast] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log format ["   + beach start - first beach position is %1",_positionLast];
			#endif
			_shoreLineArr pushBackUnique _positionLast;
			_lastPosition = "water";
			_gotInitialPosition = true;
		};
		if ((_initialPosition isEqualTo "water") && (_isSurfaceWater isEqualTo false) && !(_position isEqualTo [0,0,0])) then
		{
			#ifdef __DEBUG__
				format ["   + water start - first beach position is %1",_positionLast] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log format ["   + water start - first beach position is %1",_positionLast];
			#endif
			_shoreLineArr pushBackUnique _positionLast;
			_lastPosition = "beach";
			_gotInitialPosition = true;
	...
	if (_gotLastPosition isEqualTo false) then
	{
		if ((_lastPosition isEqualTo "beach") && (_isSurfaceWater isEqualTo true) && !(_position isEqualTo [0,0,0])) then
		{
			#ifdef __DEBUG__
				format ["   + beach finish - last beach position is %1",_positionLast] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log format ["   + beach finish - last beach position is %1",_positionLast];
			#endif
			_shoreLineArr pushBackUnique _positionLast;
			_gotLastPosition = true;
		};
		if ((_lastPosition isEqualTo "water") && (_isSurfaceWater isEqualTo false) && !(_position isEqualTo [0,0,0])) then
		{
			#ifdef __DEBUG__
				format ["   + water finish - last beach position is %1",_positionLast] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log format ["   + water finish - last beach position is %1",_positionLast];
			#endif
			_shoreLineArr pushBackUnique _positionLast;
			_gotLastPosition = true;
		};
	};
	if (_gotLastPosition isEqualTo true) exitWith {};
};
if ((_gotInitialPosition isEqualTo true) && (_gotLastPosition isEqualTo true)) then
{
	_shoreDirection = [(_shoreLineArr select 0),(_shoreLineArr select 1)] call BIS_fnc_dirTo;
	#ifdef __DEBUG__
		format ["   + shore direction is %1",_shoreDirection] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
		diag_log format ["   + shore direction is %1",_shoreDirection];
	#endif
	if (_initialPosition isEqualTo "beach") then
	{
		_taskDirection = (_shoreDirection + 90);
	};
	if (_initialPosition isEqualTo "water") then
	{
		_taskDirection = (_shoreDirection - 90);
	};
}
...
{
	_shoreDirection = round(random 360);
	#ifdef __DEBUG__
		format ["   + random shore direction is %1",_shoreDirection] remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
		diag_log format ["   + random shore direction is %1",_shoreDirection];
	#endif
};
///////////////////////////////////////////////////////////////////////////////////////////////////
_baseChosen = selectRandom _taskBase;
_baseComposition = call compile preProcessFileLineNumbers format ["bases\sub\shore\%1.sqf",_baseChosen];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
_buildingsArr = [];
_buildingsArr = nearestObjects [_taskLocation,["Building","House","HouseBase","Ruins","House_F","Ruins_F","House_Small_F"],_taskAreaBase];
_locationBldPosArr = [];
...
[_task,_taskMkr,_taskMkrExact,_taskUnits,_taskGroups] spawn
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	_scriptName = format ["tasksShore: Respawn Handler For Task %1, Units %2, Groups %3",_task,_taskUnits,_taskGroups];
