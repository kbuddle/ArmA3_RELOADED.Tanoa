...
///////////////////////////////////////////////////////////////////////////////////////////////////
#include "tasksRoadConfig.sqf"; // configuration for tasks.
///////////////////////////////////////////////////////////////////////////////////////////////////
_taskChosen = [];
...
if ((_taskChosen select 0) in RELO_var_locationTasksActive) exitWith
{
	...
	#endif
	if (isNil "noSuitableTasksRoad") then
	{
		noSuitableTasksRoad = 1;
	}
	else
	{
		noSuitableTasksRoad = noSuitableTasksRoad + 1;
		if (noSuitableTasksRoad > count _taskArr) exitWith
		{
			RELO_var_locationTaskAmount = RELO_var_locationTaskAmount - 1;
			noSuitableTasksRoad = nil;
			#ifdef __DEBUG__
				"  ! exhausted suitable road tasks, adjusted total task amount!" remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log "  ! exhausted suitable road tasks, adjusted total task amount!";
			#endif
		};
	};
};
...
///////////////////////////////////////////////////////////////////////////////////////////////////
if (_taskTerrain isEqualTo "road") then
{
	...
	if ((count _nearRoads) > 0) then
	{
		...
		{
			_nearRoadFrom = _connectedRoads select 0;
			_nearRoadTo = _connectedRoads select 1;
			if !(isNil "_nearRoadTo") then
			{
				_taskDirection = [_nearRoadFrom, _nearRoadTo] call BIS_fnc_DirTo;
			};
		}
		else
		{
			_nearRoadFrom = _nearRoads select 0;
			_nearRoadTo = _nearRoads select 1;
			if !(isNil "_nearRoadTo") then
			{
				_taskDirection = [_nearRoadFrom, _nearRoadTo] call BIS_fnc_DirTo;
			};
...
_baseComposition = call compile preProcessFileLineNumbers format ["bases\sub\road\%1.sqf",_baseChosen];
...
[_task,_taskMkr,_taskMkrExact,_taskUnits,_taskGroups] spawn
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	_scriptName = format ["tasksRoad: Respawn Handler For Task %1, Units %2, Groups %3",_task,_taskUnits,_taskGroups];
