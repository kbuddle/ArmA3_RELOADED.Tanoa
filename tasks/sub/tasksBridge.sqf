...
///////////////////////////////////////////////////////////////////////////////////////////////////
#include "tasksBridgeConfig.sqf"; // configuration for tasks.
///////////////////////////////////////////////////////////////////////////////////////////////////
_taskChosen = [];
...
if ((_taskChosen select 0) in RELO_var_locationTasksActive) exitWith
{
	...
	#endif
	if (isNil "noSuitableTasksBridge") then
	{
		noSuitableTasksBridge = 1;
	}
	else
	{
		noSuitableTasksBridge = noSuitableTasksBridge + 1;
		if (noSuitableTasksBridge > count _taskArr) exitWith
		{
			RELO_var_locationTaskAmount = RELO_var_locationTaskAmount - 1;
			noSuitableTasksBridge = nil;
			#ifdef __DEBUG__
				"  ! exhausted suitable bridge tasks, adjusted total task amount!" remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log "  ! exhausted suitable bridge tasks, adjusted total task amount!";
			#endif
		};
	};
};
...
///////////////////////////////////////////////////////////////////////////////////////////////////
if (_taskTerrain isEqualTo "bridge") then
{
	{
		_object = _x;
		if (["bridgewooden_", getModelInfo _object select 0] call BIS_fnc_inString) exitWith
		{
			_taskDirection = getDir _object;
		};
	} forEach (nearestObjects [_taskLocation, [], 12.5]);
};
///////////////////////////////////////////////////////////////////////////////////////////////////
_baseChosen = selectRandom _taskBase;
_baseComposition = call compile preProcessFileLineNumbers format ["bases\sub\bridge\%1.sqf",_baseChosen];
...
[_task,_taskMkr,_taskMkrExact,_taskUnits,_taskGroups] spawn
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	_scriptName = format ["tasksBridge: Respawn Handler For Task %1, Units %2, Groups %3",_task,_taskUnits,_taskGroups];
