...
///////////////////////////////////////////////////////////////////////////////////////////////////
#include "tasksRunwayConfig.sqf"; // configuration for tasks.
///////////////////////////////////////////////////////////////////////////////////////////////////
_taskChosen = [];
...
if ((_taskChosen select 0) in RELO_var_locationTasksActive) exitWith
{
	...
	#endif
	if (isNil "noSuitableTasksSub") then
	{
		noSuitableTasksSub = 1;
	}
	else
	{
		noSuitableTasksSub = noSuitableTasksSub + 1;
		if (noSuitableTasksSub > count _taskArr) exitWith
		{
			RELO_var_locationTaskAmount = RELO_var_locationTaskAmount - 1;
			noSuitableTasksSub = nil;
			#ifdef __DEBUG__
				"  ! exhausted suitable sub tasks, adjusted total task amount!" remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log "  ! exhausted suitable sub tasks, adjusted total task amount!";
			#endif
		};
	};
};
...
_baseComposition = call compile preProcessFileLineNumbers format ["bases\sub\runway\%1.sqf",_baseChosen];
...
[_task,_taskMkr,_taskMkrExact,_taskUnits,_taskGroups] spawn
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	_scriptName = format ["tasksRunway: Respawn Handler For Task %1, Units %2, Groups %3",_task,_taskUnits,_taskGroups];
...
switch (_taskRoleChosen) do
{
	...
	// case "assassinate": {[_taskLocation,_taskAreaBase,_taskTitle,_taskMkr,_taskUnits,_taskGroups,_taskDefinition] spawn RELO_fnc_taskRoleAssasinate;}; // marksman or sniper class needed
...
// destroy, repair, secure, steal, dismantle, seize, deactivate, download, jammer, release, assassinate, supplydrop, vehicledrop