...
///////////////////////////////////////////////////////////////////////////////////////////////////
#include "tasksPathConfig.sqf"; // configuration for tasks.
///////////////////////////////////////////////////////////////////////////////////////////////////
_taskChosen = [];
...
if ((_taskChosen select 0) in RELO_var_locationTasksActive) exitWith
{
	...
	#endif
	if (isNil "noSuitableTasksPath") then
	{
		noSuitableTasksPath = 1;
	}
	else
	{
		noSuitableTasksPath = noSuitableTasksPath + 1;
		if (noSuitableTasksPath > count _taskArr) exitWith
		{
			RELO_var_locationTaskAmount = RELO_var_locationTaskAmount - 1;
			noSuitableTasksPath = nil;
			#ifdef __DEBUG__
				"  ! exhausted suitable path tasks, adjusted total task amount!" remoteExec ["RELO_fnc_systemChatGlobal",[0,-2] select isDedicated];
				diag_log "  ! exhausted suitable path tasks, adjusted total task amount!";
			#endif
		};
	};
};
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// if (_taskTerrain in ["road","bridge","path","runway"]) then
// {
	_nearRoads = _taskLocation nearRoads 50;
	if ((count _nearRoads) > 0) then
	{
		_nearRoad = _nearRoads select 0;
		_nearRoadNext = _nearRoads select ((count _nearRoads) - 1);
		if !(isNil "_nearRoadNext") then
		{
			_taskDirection = [_nearRoad, _nearRoadNext] call BIS_fnc_DirTo;
		};
	};
// };
///////////////////////////////////////////////////////////////////////////////////////////////////
_baseChosen = selectRandom _taskBase;
_baseComposition = call compile preProcessFileLineNumbers format ["bases\sub\path\%1.sqf",_baseChosen];
...
[_task,_taskMkr,_taskMkrExact,_taskUnits,_taskGroups] spawn
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	_scriptName = format ["tasksPath: Respawn Handler For Task %1, Units %2, Groups %3",_task,_taskUnits,_taskGroups];
